...
class CameraViewController: UIViewController, AVCapturePhotoCaptureDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UIGestureRecognizerDelegate {
    ...
    /// 统一所有滑动条格式，确保都有加减按钮
    private func unifySliderFormats() {
        print("🎨 开始统一滑动条格式...")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
            ...
            // 1. 确保容器存在
            self.ensureContainersExist()
            
            // 2. 重新创建所有滑动条，使用统一格式
            self.recreateUnifiedSliders()
    ...
    /// 重新创建所有滑动条，使用统一格式
    private func recreateUnifiedSliders() {
        createUnifiedContrastSlider()
        createUnifiedSaturationSlider()
        recreateTemperatureSlider() // 重新创建以确保一致性
    ...
    /// 重新创建色温滑动条以确保一致性
    private func recreateTemperatureSlider() {
